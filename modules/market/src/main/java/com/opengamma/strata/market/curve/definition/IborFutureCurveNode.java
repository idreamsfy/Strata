...
/**
 * A curve node whose instrument is a Ibor Futures.
 */
@BeanDefinition
public class IborFutureCurveNode
  ...
  public double initialGuess(LocalDate valuationDate, ObservableValues marketData, ValueType valueType) {
    if (ValueType.ZERO_RATE.equals(valueType)) {
      return 1.0 - marketData.getValue(rateKey);
    }
    if (ValueType.DISCOUNT_FACTOR.equals(valueType)) {
      double approximateMaturity = template.getPeriodToStart()
          .plus(template.getConvention().getIndex().getTenor()).toTotalMonths() / 12.0d;
      return Math.exp(-approximateMaturity * (1.0 - marketData.getValue(rateKey)));
    }
    return 0d;
  }
